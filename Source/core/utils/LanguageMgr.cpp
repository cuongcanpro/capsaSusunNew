...
static const std::string LANGUAGE_DEFAULT = LANGUAGE_VN;
...
static const std::string KEY_CURRENT_LANGUAGE = "game_current_language_select";
	...
	while (std::getline(fileStringStream, line))
	{
			...
			line.find("*/", 0) == string::npos) //filter the valid string of one line
		{
			...
			if (validPos != string::npos)
			{
				...
				AXLOGD("string localize %s %s", keyStr.c_str(), subStr.c_str());
...
void LocalizedImage::loadLocalized(std::string path)
{
	...
	// Get data of file
	contents = FileUtils::getInstance()->getStringFromFile("Board/lang/Localized_content.txt");
...
void LocalizedImage::setPathLanguage(std::string path) {
	patchLanguage = path;
}
...
void LanguageMgr::addLanguage(std::string lang)
{
	if (validateLanguage(lang)) {
		...
		imgLocal->setPathLanguage(LANGUAGES_PATH.find(lang)->second);
...
{
	return "Board/lang/Localized_" + lang + ".txt";
}
...
{
	languageMgr->addLocalizedText(path, LANGUAGE_VN);
}
